		        ...
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/cat_products/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_cat_products = !show_list_cat_products">
		                        <div class="switch_box">
		                            <span v-if="show_list_cat_products"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!--猫猫用品推荐列表组件 -->
		                <list_cat_products v-if="show_list_cat_products" :list="list_cat_products" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_cat_products.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_cat_products" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_cat_products" :key="i" @click="to_details('cat_products',o,'cat_products_id')">
		                            <td v-for="(oj,n) in fields_cat_products">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
...
<script>
	...
	import list_cat_products from "@/components/diy/list_cat_products.vue";
	...
	export default {
		...
		methods: {
			...
			// 获取猫猫用品列表
			get_cat_products() {
				let url = "~/api/cat_products/get_list?";
				this.$get(url, {
					"page": 1,
					"size": 4
				}, (json) => {
					if (json.result) {
						this.list_cat_products = json.result.list;
					}
				})
			},
		...
		computed: {
		    list_table_cat_products() {
		        var list = this.list_cat_products;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_cat_products.map((o) => {
		                // 第二个中括号是对象的属性
