<template>
	<div class="diy_details page_cat_products" id="cat_products_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_cat_products'>
							<div_cat_products :obj="obj"></div_cat_products>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_cat_products from "@/components/diy/div_cat_products.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/cat_products/set?cat_products_id=' + obj["cat_products_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							source_table: "cat_products",
							source_field: "cat_products_id",
							source_id: this.obj.cat_products_id,
			...
			get_obj_after(json) {
				...
				if (json && json.result && json.result.obj) {
					...
					this.form_comment.source_id = this.obj.cat_products_id;
				}
			},
			/**
			 * 初始化收藏状态
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.commodity_price
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/cat_products/set?cat_products_id=' + _this.obj["cat_products_id"], {
								praise_len
							}, (res) => {
								if(res.result){
