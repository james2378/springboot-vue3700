<template>
	<div class="diy_details page_service_centre" id="service_centre_details">
		<div class='warp'>
			<div class='container'>
				<div class='row'>
					<div class='col'>
						<div class='card_service_centre'>
							<div_service_centre :obj="obj"></div_service_centre>
						</div>
					</div>
				</div>
			</div>
...
<script>
	import div_service_centre from "@/components/diy/div_service_centre.vue";
	...
	export default {
		...
		methods:{
			...
			add_hits(obj) {
				obj["hits"] = obj["hits"] + 1;
				var hits = obj["hits"];
				this.$post('~/api/service_centre/set?service_centre_id=' + obj["service_centre_id"], {
					hits
				}, (res) => {
					if(res.result){
						...
						var body = {
							...
							source_field: "service_centre_id",
							source_id: this.obj.service_centre_id,
			...
			get_collect(obj) {
				...
				var query = {
					source_table: "service_centre",
					source_field: "service_centre_id",
					source_id: obj["service_centre_id"],
					user_id
				};
		 ...
		 /****
			...
			change_collect() {
				...
				if (this.state_collect) {
					...
					query.title = this.obj.single_price
					query.title = this.obj.contacts
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/service_centre/set?service_centre_id=' + _this.obj["service_centre_id"], {
								praise_len
							}, (res) => {
								if(res.result){
