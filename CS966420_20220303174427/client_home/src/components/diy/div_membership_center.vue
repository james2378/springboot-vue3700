<template>
	<div class="diy_details_box diy_div_membership_center">
		<div class="warp">
			<div class="container">
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					...
					source_field: "membership_center_id",
					source_id: this.obj["membership_center_id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/membership_center/set?membership_center_id=' + _this.obj["membership_center_id"], {
								praise_len
							}, (res) => {
								if(res.result){
