<template>
	<div class="diy_details_box diy_div_booking_management_">
		<div class="warp">
			<div class="container">
	...
	export default {
		...
		methods: {
			...
			change_praise(obj) {
				...
				var query = {
					source_table: "booking_management_",
					source_field: "booking_management__id",
					source_id: this.obj["booking_management__id"],
					user_id
				};
				...
				if (this.state_praise) {
					...
					this.$get('~/api/praise/del?', query, (res) => {
						if(res.result){
							...
							this.$post('~/api/booking_management_/set?booking_management__id=' + _this.obj["booking_management__id"], {
								praise_len
							}, (res) => {
								if(res.result){
