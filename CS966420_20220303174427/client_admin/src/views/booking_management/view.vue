		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','service_type') || $check_field('add','service_type') || $check_field('set','service_type')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="服务类型" prop="service_type">
					<el-input id="service_type" v-model="form['service_type']" placeholder="请输入服务类型"
							  v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','service_type')) || (!form['booking_management_id'] && $check_field('add','service_type'))" :disabled="disabledObj['service_type_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','shop_address') || $check_field('add','shop_address') || $check_field('set','shop_address')" :xs="24" :sm="12" :lg="8">
				<el-form-item label=" 店铺地址" prop="shop_address">
					<el-input id="shop_address" v-model="form['shop_address']" placeholder="请输入 店铺地址"
							  v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','shop_address')) || (!form['booking_management_id'] && $check_field('add','shop_address'))" :disabled="disabledObj['shop_address_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','contacts') || $check_field('add','contacts') || $check_field('set','contacts')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="联系人" prop="contacts">
					<el-input id="contacts" v-model="form['contacts']" placeholder="请输入联系人"
							  v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','contacts')) || (!form['booking_management_id'] && $check_field('add','contacts'))" :disabled="disabledObj['contacts_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','contact_number') || $check_field('add','contact_number') || $check_field('set','contact_number')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="联系电话" prop="contact_number">
					<el-input id="contact_number" v-model="form['contact_number']" placeholder="请输入联系电话"
							  v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','contact_number')) || (!form['booking_management_id'] && $check_field('add','contact_number'))" :disabled="disabledObj['contact_number_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','single_price') || $check_field('add','single_price') || $check_field('set','single_price')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="单次价格" prop="single_price">
					<el-input id="single_price" v-model="form['single_price']" placeholder="请输入单次价格"
							  v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','single_price')) || (!form['booking_management_id'] && $check_field('add','single_price'))" :disabled="disabledObj['single_price_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','service_times') || $check_field('add','service_times') || $check_field('set','service_times')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="服务次数" prop="service_times">
					<el-input-number id="service_times" v-model.number="form['service_times']"
						v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','service_times')) || (!form['booking_management_id'] && $check_field('add','service_times'))"></el-input-number>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','total_price') || $check_field('add','total_price') || $check_field('set','total_price')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="总价格" prop="total_price">
					<el-input id="total_price" v-model="form['total_price']" placeholder="请输入总价格"
							  v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','total_price')) || (!form['booking_management_id'] && $check_field('add','total_price'))"  @focus="set_total_price()" :disabled="disabledObj['total_price_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','time_of_appointment') || $check_field('add','time_of_appointment') || $check_field('set','time_of_appointment')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="预约时间" prop="time_of_appointment">
					<el-date-picker :disabled="disabledObj['time_of_appointment_isDisabled']" v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','time_of_appointment')) || (!form['booking_management_id'] && $check_field('add','time_of_appointment'))" id="time_of_appointment"
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','user_account') || $check_field('add','user_account') || $check_field('set','user_account')" :xs="24" :sm="12" :lg="8">
				<el-form-item label="用户账号" prop="user_account">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','user_account')) || (!form['booking_management_id'] && $check_field('add','user_account'))" :disabled="disabledObj['user_account_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','user_account')">{{form['user_account']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['booking_management_id'] && $check_field('set','user_account')) || (!form['booking_management_id'] && $check_field('add','user_account'))" id="user_account" v-model="form['user_account']" :disabled="disabledObj['user_account_isDisabled']">
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "booking_management_id",
				url_add: "~/api/booking_management/add?",
				url_set: "~/api/booking_management/set?",
				url_get_obj: "~/api/booking_management/get_obj?",
				url_upload: "~/api/booking_management/upload?",

				query: {
					"booking_management_id": 0,
				},
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/booking_management/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/booking_management/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/booking_management/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/booking_management/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/booking_management/view','get');
