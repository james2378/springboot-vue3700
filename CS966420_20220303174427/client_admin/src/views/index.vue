				...
				<el-row>
					<el-col v-if="user_group == '管理员' || $check_figure('/order_center/table')" :span="8">
						<div class="card chart">
							<newBarChart v-if="bar_obj_order_center.values.length > 0" id="bar_obj_order_center" :vm="bar_obj_order_center" :title="'订单中心统计'">
							</newBarChart>
							<div v-if="!bar_obj_order_center.values.length">订单中心没有符合条件的数据</div>
						</div>
					</el-col>
				</el-row>
			</div>
	...
	export default {
		...
		methods: {
			...
			async get_list_order_center() {
				...
				this.bar_obj_order_center.names = name_list
				...
				await this.$get(
						"~/api/order_center/bar_group?field="+query_str+"&groupby="+group_by_value,
						data,
						(json) => {
							if (json.result) {
								let xAxis = [];
								...
								this.bar_obj_order_center.xAxis = xAxis;
								this.bar_obj_order_center.values = values;
							}
							if (flag){
								this.get_nickname(this.bar_obj_order_center.xAxis,true);
							}
						});
			},
				getUserInfo(){
